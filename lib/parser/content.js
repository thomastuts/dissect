 ...
 * @returns {*}
 */
function getLikelyCandidateForItemProp(rawHtml) {
    if(!rawHtml){
        return '';
    }
    var $ = cheerio.load(rawHtml);
    var $rel = $('.relacionadas').remove();
    var $body = $('div[itemprop="articleBody"]');
    return $('div[itemprop="articleBody"]').text()
}
/**
...
function getArticleContentForItemprop(rawHtml, host) {
    var content = getLikelyCandidateForItemProp(rawHtml);
    content = cleaner.cleanAfterParsing(content, host);
    return content;
}
